...
void reset() {
	posOut = texture(sResetData, vUV.st);
}
...
void main() {
	...
	float actualStepSize = uStepSize * mix(1., uStepSizeMultiplier,
		smoothstep(uTrailThresholds.x, uTrailThresholds.y, newData.r));
	...
	stateOut.r += uAgeStep;
	stateOut.b = heading;
	stateOut.a = doDeposit*uDepositAmt;
}
